...
- [x] **UI 细节优化 (2026-01-10)**
  - [x] **Index 动画**: 活跃状态由"猫咪"改为**"翻书动画"** (Book Flip)，居中显示。
  - [x] **结算动画**: 红色对勾调整至进度条尾部，避免遮挡。
- [x] **多轮专注功能 (2026-01-13)**
  - [x] **弹幕命令**: 支持 `打卡 <项目> <时长> <N>次` (例如: `打卡 学习 45 3次`)。
  - [x] **自动休息**: 轮次间自动进入休息状态，倒计时结束后开始下一轮。
  - [x] **UI 增强**: `index.html` 支持休息状态蓝条显示，并添加 `×N` 剩余轮数角标。
  - [x] **灵活配置**: `.env` (`BREAK_DURATION`) 和管理后台均可配置休息时长。
  - [x] **Bug Fix**: 修复了省略时间单位时正则解析失效的问题。
- [x] **AI 配置与体验升级 (2026-02-04)**
  - [x] **多厂商支持**: 集成 OpenAI, DeepSeek, SiliconFlow, Moonshot, Aliyun (Qwen), ZhipuAI (GLM), Gemini。
  - [x] **配置隔离 (Profile Isolation)**: 管理后台可按服务商自动保存/恢复 API Key、模型列表和选中状态。
  - [x] **智能连通性测试**: 支持自动获取模型列表，若不支持则降级为 Key 有效性验证。
  - [x] **后台 UI 优化**: 重构 `admin.html` 布局，支持自动换行，优化了移动端和桌面端体验。
  - [x] **后台 UI 优化**: 重构 `admin.html` 布局，支持自动换行，优化了移动端和桌面端体验。
  - [x] **无限正计时模式**: 支持无限时长的正计时，避免了轮次限制。
- [x] **数据安全加强**:
  - [x] **自动备份**: 每次系统启动时自动备份数据库 (`backups/daka_YYYY-MM-DD_HH-mm-ss.db`)。
  - [x] **Git 规范**: 完善 `.gitignore` 规则，防止数据库和日志被意外提交。
- [x] **Bug 修复与安全加固 (2026-02-13)**
  - [x] **凭证安全**: 移除 `src/config/index.ts` 中硬编码的 Bilibili API 凭证 (appId, appKey, appSecret, accessToken, roomId)，改为空值 fallback，强制从 `.env` 读取。
...
## 数据库统计 (Database Statistics 2026-02-04)
